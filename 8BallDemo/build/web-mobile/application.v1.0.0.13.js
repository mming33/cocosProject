System.register([], function (_export, _context) {
  ...
  function createApplication(_ref) {
    ...
    function loadSettingsJson(cc) {
      return new Promise(function (resolve, reject) {
        setLoadingDisplay();
        cc.loader.load('./res/settings.c9551.json', function (err, json) {
          if (err) {
            return reject(err);
          }
  ...
  function onGameStarted(cc, settings) {
    ...
    cc.director.loadScene(launchScene, null, function () {
      cc.view.setDesignResolutionSize(1700, 750, 4);
  ...
  function setLoadingDisplay() {
    ...
    var proNum = 0;
    var percentNum = 0;
    var interTime = setInterval(() => {
      proNum += 2;
      console.log("proNum:" + proNum + "    percentNum:" + percentNum);
      if (proNum > percentNum) {
        progressBar.style.height = proNum + '%';
      }
      if (proNum >= 10 || percentNum >= 10) {
    ...
    cc.loader.onProgress = function (completedCount, totalCount, item) {
      if (totalCount > 10) {
        ...
        percentNum = percent;
        if (progressBar && percentNum > proNum) {
